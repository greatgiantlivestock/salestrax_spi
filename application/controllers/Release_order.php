...
class Release_order extends CI_Controller {
	...
	public function save() {
		if($this->session->userdata('id_role') == "1" || $this->session->userdata('id_role') == "4" || $this->session->userdata('id_role') == "6") {
			...
			$in3['id_customer'] 	= $this->input->post('cust_ship');
			...
			$pers_numb 	= $this->input->post('pers_numb1');
			$insL['matgr'] 	= $this->input->post('matgr');
			$insL['pers_numb'] 	= $this->input->post('pers_numb1');
			$insL['matdesc'] 	= $qmatgr->matdesc;
			$qship = $this->db->query("SELECT count(*) as jml FROM mst_shipping_point_customer WHERE kode_customer='$kode_customer' AND
									id_shipping_point='$id_shipping_point' AND matgr='$matgr'")->row();
			$qdiv = $this->db->query("SELECT count(*) as jml1 FROM mst_sales_person_division WHERE kode_customer='$kode_customer' AND
									pers_numb='$pers_numb' AND matgr='$matgr'")->row();
			if($qship->jml=='0'){
			...
			if($this->input->post("ck_id_detail") != ''){
				foreach($this->input->post("ck_id_detail") as $data_id) {
					...
					foreach($qdata1->result_array() as $rows){
						...
						$content = substr($data1,0,10).$data2.$data3.$data4.substr($data5,0,20).$data6a.$data7a.substr($data8,0,15).substr($data9c,-8).substr($data10,0,3).substr($data11,0,4).$data12.$data13."\n";
