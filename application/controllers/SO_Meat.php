...
class SO_Meat extends CI_Controller {
	public function index($id="",$idt="") {
		...
		if($this->session->userdata('id_role') == 1 || $this->session->userdata('id_role') == 4 || $this->session->userdata('id_role') == 5 || $this->session->userdata('id_role') == 6) {
			...
			if($id != "") { 
				...
				if($get_id->num_rows() > 0) {
					foreach($get_id->result() as $data) {
						...
						$d['combo_nomor_order'] = $this->App_model->get_combo_nomor_order_meat($id);
					...
					$d['btn_batal'] = '<a class="btn btn-xs btn-default"  href="'.base_url().'SO_Meat">
					...
					$d['tipe'] = 'edit';
					if($idt != '') {
						$get_edit = $this->db->query("SELECT * FROM trx_so_detail WHERE id_request='$id'");
						if($get_edit->num_rows() > 0) {
							foreach($get_edit->result() as $data_edit) {
								...
								$d['combo_product'] = $this->App_model->get_combo_product_meat($data_edit->id_product);
								...
								$d['btn_batal_edit'] = '<a class="btn btn-xs btn-danger" style="margin-left: 40px;" href="'.base_url().'SO_Meat/index/'.$id.'">
											<i class="ace-icon fa fa-close"></i>
											<span class="bigger-110">Batal</span>
										</a>';
								$d['tipe_detail'] = 'edit';
						...
						$d['combo_product'] = $this->App_model->get_combo_product_meat();
				...
				$d['combo_nomor_order'] = $this->App_model->get_combo_nomor_order_meat();
			...
			$this->load->view('menu');
			$this->load->view('so_daging/so_customer_table');
		...
		if($this->session->userdata('id_role') == "1" || $this->session->userdata('id_role') == "4" || $this->session->userdata('id_role') == "5" || $this->session->userdata('id_role') == "6") {
			...
			if($tipe == "add") {
				...
				if($error) {
					...
					$qVNo = $this->db->query("SELECT count(*) as countN FROM trx_so_header WHERE no_po='$Npo' and tanggal_kirim='$tanggal_kirim'")->row();
					if($Npo==''){
							...
							if($qship == '' || $qsold ==''){
								...
								if($this->upload->do_upload("file_upload")) {
									...
									*/
									...
									redirect("SO_Meat/index/".$last_id);
				...
				if($error) {
					$this->session->set_flashdata("error","Mohon mengisi data dengan lengkap");
					redirect("SO_Meat/index/".$this->input->post('id_request'));	
				} else {
					$tanggal_request=date("Y-m-d");
					$tanggal_kirim=$this->input->post('tanggal_kirim');
					if($tanggal_kirim < $tanggal_request){
	...
	public function salin() {
		if($this->session->userdata('id_role') == "1" || $this->session->userdata('id_role') == "3" || $this->session->userdata('id_role') == "4"  || $this->session->userdata('id_role') == "5") {
			...
			if($qValidate->tanggal_kirim == $tanggal_kirim){
				...
				redirect("SO_Meat/index/".$id_requestCopy);
				...
				redirect("SO_Meat/index/".$last_idCopy);
			}			
		} else {
			redirect("login");
		}
		...
		if($this->session->userdata('id_role') == "1" || $this->session->userdata('id_role') == "4" || $this->session->userdata('id_role') == "5" || $this->session->userdata('id_role') == "6") {
			...
			if($tipe == "add") {
				...
				// 	}else{
						...
						$in['keterangan'] = $this->input->post('keterangan');
						$in['id_request'] 	= $this->input->post('id_request');	
						// $id_customer_dt = $this->input->post('id_customer_dt');	
						// $id_product = $this->input->post('id_product');
						// $qspaut = $this->db->query("SELECT id_sales_person,pers_numb FROM(SELECT sp.id_sales_person,sp.pers_numb FROM sales_person sp 
						// JOIN mst_customer mc ON sp.pers_numb=mc.pers_numb1 JOIN mst_plant_group mpg ON mpg.id_plant_group=sp.id_plant_group JOIN mst_product mp 
						// ON mp.id_plant_group=mpg.id_plant_group WHERE mp.id_product='$id_product' AND mc.id_customer='$id_customer_dt')AS data1 UNION
						// SELECT sp.id_sales_person,sp.pers_numb FROM sales_person sp JOIN mst_customer mc ON sp.pers_numb=mc.pers_numb2 JOIN mst_plant_group mpg 
						// ON mpg.id_plant_group=sp.id_plant_group JOIN mst_product mp ON mp.id_plant_group=mpg.id_plant_group WHERE mp.id_product='$id_product' 
						// AND mc.id_customer='$id_customer_dt'")->row();
						$matgr = $qsatuan->matgr;
